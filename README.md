...
* **BancoController:** Controlador que gerencia as operações bancárias de um cliente, incluindo consultas de saldo, depósitos e saques.
* **RelatorioController:** Controlador responsável pela geração e exportação de relatórios de usuários em formato CSV.
* **UsuarioController:** Controlador que gerencia a autenticação de usuários, lidando com o login de clientes e funcionários e a navegação entre as interfaces de usuário.
### dao
...
* **BancoDAO:** Classe que gerencia operações relacionadas a contas bancárias, como consultar saldo, realizar depósitos e saques.
* **ClienteDAO:** Classe de acesso a dados para operações relacionadas a clientes, incluindo inserção e consulta de usuários.
* **ConnectionFactory:** Classe responsável por gerenciar a conexão com o banco de dados, criando e retornando novas conexões.
* **FuncionarioDAO:** Classe de acesso a dados para operações relacionadas a funcionários, como inserção de contas e consulta de dados.
### models
Pasta que define as classes de modelo (entidades) utilizadas na aplicação.
* **Cliente:** Classe que representa um cliente do banco, contendo informações pessoais e métodos para manipulação de dados.
* **Conta:** Classe abstrata que define a estrutura básica de uma conta bancária.
* **ContaCliente:** Classe que representa uma conta de cliente.
* **Endereco:** Classe que representa o endereço de um usuário.
* **RelatorioUsuario:** Classe que representa o relatório de um usuário.
* **Transacao:** Classe que representa uma transação bancária.
...
* **CSVExporter:** Classe utilitária para exportar listas de transações para arquivos CSV.
* **DBUtils:** Classe que contém métodos utilitários para operações relacionadas ao banco de dados, como inserção de novos usuários e manipulação de dados.
* **DataManager:**  Classe para gerenciar dados temporários ou realizar operações de manipulação de dados
### views
Pasta que contém as interfaces de usuário (UI) da aplicação.
* **CadastroView:** Classe que representa a interface gráfica para o cadastro de novos usuários.
* **ClienteView:** Classe que representa a interface gráfica para clientes do banco.
* **FuncionarioView:** Classe que representa a interface gráfica para os funcionários do banco.
* **MainView:** Classe que representa a tela principal do aplicativo.
* **PasswordDialogView:** Classe que apresenta um diálogo para autenticação de usuários, solicitando email e senha.
